...
CREATE TABLE `implementation` (
  ...
  `status` varchar(32) NOT NULL COMMENT 'values: NEW, READY,  FAILED, PUBLIC, OLD, DEPRECATED, COMPROMISED',
...
CREATE TABLE uploaded_file (
    `idWEFile` int not null auto_increment,
    `name` varchar(300),
...
CREATE TABLE job_manager (
    `jobManagerId` int not null auto_increment,
    `jobManagerName` varchar(50),
    PRIMARY KEY(`jobManagerId`)
) ENGINE = InnoDB;

CREATE TABLE backend (
    `idBackend` int not null auto_increment,
    `backendName` varchar(50),
    `backendDesc` varchar(5000),
...
CREATE TABLE supported_job_managers (
    `jobManagerId` int not null,
    `idBackend` int not null,
    FOREIGN KEY(`jobManagerId`) REFERENCES `job_manager`(`jobManagerId`),
    FOREIGN KEY(`idBackend`) REFERENCES `backend`(`idBackend`)
) ENGINE = InnoDB;

CREATE TABLE workflow_engine_files (
    `idWE` int not null,
    `idWEFile` int not null,
    FOREIGN KEY(`idWE`) references `platform`(`id`),
    FOREIGN KEY(`idWEFile`) references `uploaded_file`(`idWEFile`)
) ENGINE = InnoDB;

CREATE TABLE be_instance (
    `idBackendInst` int not null auto_increment,
    `backendInstName` varchar(50),
    `idBackend` int not null,
    `site` varchar(100),
    `backendOutput` varchar(200),
    `backendErrorOut` varchar(200),
    `resource` varchar(100),
    `jobManager` varchar(50),
    `jobTypeId` int,
    `idOS` int not null,
    `we_dev` int not null,
    PRIMARY KEY(`idBackendInst`),
    FOREIGN KEY(`idOS`) REFERENCES `operating_systems`(`idOS`),
    FOREIGN KEY(`we_dev`) REFERENCES `repo_user`(`id`),
    FOREIGN KEY(`idBackend`) references `backend`(`idBackend`),
    FOREIGN KEY(`jobTypeId`) references `job_type`(`jobTypeId`)
) ENGINE = InnoDB;

CREATE TABLE we_implementation (
    `idWEImp` int not null auto_increment,
    `nameWEImp` varchar(50),
    `descriptionWEImp` varchar(5000),
    `preDeployed` tinyint(1),
    `shellPath` varchar(100),
    `shellWEFileId` int,
    `zipWEFileId` int,
    `prefixData` varchar(500),
    `idWE` int not null,
    `idBackendInst` int not null,
    `we_dev` int not null,
    PRIMARY KEY(`idWEImp`),
    FOREIGN KEY(`idWE`) REFERENCES `platform`(`id`),
    FOREIGN KEY(`we_dev`) REFERENCES `repo_user`(`id`),
    FOREIGN KEY(`idBackendInst`) REFERENCES `be_instance`(`idBackendInst`),
    FOREIGN KEY(`shellWEFileId`) REFERENCES `uploaded_file`(`idWEFile`),
    FOREIGN KEY(`zipWEFileId`) REFERENCES `uploaded_file`(`idWEFile`)
) ENGINE = InnoDB;
...
/*!50001 VIEW `web_role` AS select `u`.`login_name` AS `login_name`,`g`.`role_name` AS `role_name` from (`repo_user` `u` join `role_flags` `g`) where ((`u`.`active` = 1) and (((`u`.`admin` = 1) and (`g`.`admin` = '1')) or ((`u`.`we_dev` = 1) and (`g`.`we_dev` = 1)) or ((`g`.`we_dev` = 0) and (`g`.`admin` = 0)))) */;
